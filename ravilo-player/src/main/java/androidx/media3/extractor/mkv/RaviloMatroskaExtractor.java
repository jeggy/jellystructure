...
// the same way upstream already follows it to Cues. On a Media3 upgrade, re-copy the new upstream
// file and re-apply the marked hunks; RaviloMatroskaExtractorVersionTest fails until you do.
...
@UnstableApi
public class RaviloMatroskaExtractor implements Extractor {

  /**
   * Creates a factory for {@link RaviloMatroskaExtractor} instances with the provided {@link
   * SubtitleParser.Factory}.
   */
  public static ExtractorsFactory newFactory(SubtitleParser.Factory subtitleParserFactory) {
    return () -> new Extractor[] {new RaviloMatroskaExtractor(subtitleParserFactory)};
  }
  ...
  public static final ExtractorsFactory FACTORY =
      () ->
          new Extractor[] {
            new RaviloMatroskaExtractor(SubtitleParser.Factory.UNSUPPORTED, FLAG_EMIT_RAW_SUBTITLE_DATA)
          };

  private static final String TAG = "RaviloMatroskaExtractor";
  ...
  // RAVILO R294 — where SeekHead says Tracks is, whether it has been parsed, and the Cluster position
  // to return to after fetching it from the tail of the file.
  private long tracksContentPosition = C.INDEX_UNSET;
  private boolean seenTracks;
  private boolean seekForTracks;
  private long seekPositionAfterBuildingTracks = C.INDEX_UNSET;
  ...
  /**
   * @deprecated Use {@link #RaviloMatroskaExtractor(SubtitleParser.Factory)} instead.
   */
  @Deprecated
  public RaviloMatroskaExtractor() {
    this(new DefaultEbmlReader(), FLAG_EMIT_RAW_SUBTITLE_DATA, SubtitleParser.Factory.UNSUPPORTED);
  }

  /**
   * @deprecated Use {@link #RaviloMatroskaExtractor(SubtitleParser.Factory, int)} instead.
   */
  @Deprecated
  public RaviloMatroskaExtractor(@Flags int flags) {
    this(
   ...
   *     extraction.
   */
  public RaviloMatroskaExtractor(SubtitleParser.Factory subtitleParserFactory) {
   ...
   */
  public RaviloMatroskaExtractor(SubtitleParser.Factory subtitleParserFactory, @Flags int flags) {
    this(new DefaultEbmlReader(), flags, subtitleParserFactory);
  }

  /* package */ RaviloMatroskaExtractor(
  ...
  public final int read(ExtractorInput input, PositionHolder seekPosition) throws IOException {
    ...
    while (continueReading && !haveOutputSample) {
      ...
      if (continueReading && maybeSeek(seekPosition, input.getPosition())) { // RAVILO R294
  ...
  protected void startMasterElement(int id, long contentPosition, long contentSize)
    ...
    switch (id) {
      ...
      case ID_CUE_POINT:
        ...
        break;
      case ID_CLUSTER:
        // RAVILO R294 — a Cluster before any Tracks, with Tracks further on: go and get it first.
        if (!seenTracks
            && tracksContentPosition != C.INDEX_UNSET
            && tracksContentPosition > contentPosition) {
          seekForTracks = true;
          break;
        }
        if (!sentSeekMap) {
  ...
  protected void endMasterElement(int id) throws ParserException {
    ...
    switch (id) {
      ...
      case ID_SEEK:
        ...
        } else if (seekEntryId == ID_TRACKS) { // RAVILO R294
   ...
   */
  // RAVILO R294 — Tracks first, then the unchanged Cues logic. After Tracks has been read from the
  // tail, the Cues decision the first Cluster skipped is taken here: fetch Cues (returning to the
  // Cluster afterwards, as upstream does) or declare the file unseekable, then go back to the Cluster.
  private boolean maybeSeek(PositionHolder seekPosition, long currentPosition) {
    if (seekForTracks) {
      seekForTracks = false;
      seekPositionAfterBuildingTracks = currentPosition;
      seekPosition.position = tracksContentPosition;
      return true;
    }
    if (seenTracks && seekPositionAfterBuildingTracks != C.INDEX_UNSET) {
      long returnPosition = seekPositionAfterBuildingTracks;
      seekPositionAfterBuildingTracks = C.INDEX_UNSET;
      if (!sentSeekMap) {
        if (seekForCuesEnabled && cuesContentPosition != C.INDEX_UNSET) {
          seekPositionAfterBuildingCues = returnPosition;
      ...
      seekPosition.position = returnPosition;
      return true;
    }
    return maybeSeekForCues(seekPosition, currentPosition);
  }
  ...
  /** Passes events through to the outer {@link RaviloMatroskaExtractor}. */
  private final class InnerEbmlProcessor implements EbmlProcessor {

    @Override
    public @ElementType int getElementType(int id) {
      return RaviloMatroskaExtractor.this.getElementType(id);
    }

    @Override
    public boolean isLevel1Element(int id) {
      return RaviloMatroskaExtractor.this.isLevel1Element(id);
    }

    @Override
    public void startMasterElement(int id, long contentPosition, long contentSize)
        throws ParserException {
      RaviloMatroskaExtractor.this.startMasterElement(id, contentPosition, contentSize);
    }

    @Override
    public void endMasterElement(int id) throws ParserException {
      RaviloMatroskaExtractor.this.endMasterElement(id);
    }

    @Override
    public void integerElement(int id, long value) throws ParserException {
      RaviloMatroskaExtractor.this.integerElement(id, value);
    }

    @Override
    public void floatElement(int id, double value) throws ParserException {
      RaviloMatroskaExtractor.this.floatElement(id, value);
    }

    @Override
    public void stringElement(int id, String value) throws ParserException {
      RaviloMatroskaExtractor.this.stringElement(id, value);
    }

    @Override
    public void binaryElement(int id, int contentsSize, ExtractorInput input) throws IOException {
      RaviloMatroskaExtractor.this.binaryElement(id, contentsSize, input);
    }
  }
  ...
  protected static final class Track {
     ...
     *
     * <p>It is unfortunately not possible to mark {@link RaviloMatroskaExtractor#tracks} as only
