...
tasks.register<Copy>("syncDesignAssets") {
    ...
    from(rootProject.layout.projectDirectory.dir("design/app")) {
        include("wf.css", "app.css", "detail.css", "metadata.css", "seeding.css", "seeding.js", "segments.css")
...
// ─── Phase 224 / R252 — one version string per build ────────────────────────────────────────────
// Resolved once, here, and shared with every module (:shared's BuildInfo for the clients,
// :ravilo-android's versionName, and this project's own BuildInfo for the backend). Precedence:
// -Pjellystructure.version, then -Pravilo.versionName (what deploy-play-store.yml already passes from
// the release tag), then `git describe --tags --always --dirty` with a leading "v" stripped, else "dev".
// A Docker build context has no .git (see .dockerignore), so the images resolve to "dev" here and take
// the real value at runtime from BUILD_VERSION instead (FR-224-6) — that keeps a release build of an
// already-pushed commit a full cache hit.
val buildVersion: String = run {
    val explicit = (findProperty("jellystructure.version") as String?)?.trim()?.ifBlank { null }
        ?: (findProperty("ravilo.versionName") as String?)?.trim()?.ifBlank { null }
    explicit ?: runCatching {
        providers.exec {
...
// dev.jellystructure.BuildInfo for the backend (and the admin wasm frontend — same commonMain). Wired as
// a srcDir PLUS an explicit dependency on every Kotlin compile task: the SQLDelight afterEvaluate above
// rewrites commonMain's srcDirs as plain files, which would silently drop a `builtBy` wiring exactly
// the way it once dropped SQLDelight's own (see that block's comment).
...
val generateBuildInfo by tasks.registering {
    ...
    doLast {
        ...
        file.writeText(
            "// Generated by the generateBuildInfo task (phase 224) — do not edit, do not commit.\n" +
