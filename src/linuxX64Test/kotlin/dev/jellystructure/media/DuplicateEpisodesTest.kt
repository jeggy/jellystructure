 ...
 * `Teletubbies.S01…` folder had been extracted a second time inside the S02 folder.
 */
class DuplicateEpisodesTest {
    ...
    private val teletubbies = listOf(
        ep("/m/Teletubbies/Teletubbies.S01/S01E01.mkv", 1, 1, "jf-e1"),
        ep("/m/Teletubbies/Teletubbies.S02/Teletubbies.S01/S01E01.mkv", 1, 1, "jf-e1"),
        ep("/m/Teletubbies/Teletubbies.S01/S01E02.mkv", 1, 2, "jf-e2"),
        ep("/m/Teletubbies/Teletubbies.S02/Teletubbies.S01/S01E02.mkv", 1, 2, "jf-e2"),
        ep("/m/Teletubbies/Teletubbies.S02/S02E01.mkv", 2, 1, "jf-s2e1"),
    )
    ...
    fun `duplicate files collapse to one entry per episode`() {
        assertEquals(2, DuplicateEpisodes.extraCount(teletubbies))
        val deduped = DuplicateEpisodes.deduped(teletubbies)
        assertEquals(3, deduped.size)
        // The nested re-extraction loses: the original S01 folder's path sorts first.
        assertEquals(
            listOf(
                "/m/Teletubbies/Teletubbies.S01/S01E01.mkv",
                "/m/Teletubbies/Teletubbies.S01/S01E02.mkv",
                "/m/Teletubbies/Teletubbies.S02/S02E01.mkv",
            ),
            deduped.map { it.path },
        )
    ...
    fun `no two exposed episodes share an id`() {
        val ids = DuplicateEpisodes.deduped(teletubbies).mapNotNull { it.jellyfinId }
        assertEquals(ids.distinct(), ids)
    }

    @Test
    fun `redundant copies lose the borrowed jellyfin id but keep their row`() {
        val unique = DuplicateEpisodes.withUniqueIds(teletubbies)
        assertEquals(teletubbies.size, unique.size)
        assertEquals("jf-e1", unique.first { it.path == "/m/Teletubbies/Teletubbies.S01/S01E01.mkv" }.jellyfinId)
        assertNull(unique.first { it.path == "/m/Teletubbies/Teletubbies.S02/Teletubbies.S01/S01E01.mkv" }.jellyfinId)
    ...
    fun `triage reports the duplicates as a library issue`() {
        assertEquals(2, TriageDetection.duplicateEpisodeCount(series(teletubbies)))
        assertEquals(0, TriageDetection.duplicateEpisodeCount(series(DuplicateEpisodes.deduped(teletubbies))))
    ...
    fun `describe names the episode and every colliding file`() {
        val lines = DuplicateEpisodes.describe(teletubbies)
