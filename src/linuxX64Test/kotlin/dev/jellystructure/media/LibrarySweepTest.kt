...
class LibrarySweepTest {
    ...
    fun `a missing episode resolves to its parent series id not the episode id`() {
        // Reproduces the reported Fjollerne case: the series (Jellyfin id "fjollerne") is already held, but its
        ...
        val result = computeLibraryDiff(
            jfItems = listOf(
                jf("fjollerne", "Series"),
                jf("fjollerne-s11e07", "Episode", seriesId = "fjollerne"),
            ),
            ourTopLevelIds = setOf("fjollerne"),
            ourEpisodeIds = emptySet(), // the new episode isn't in our set yet
        )
        assertEquals(listOf("fjollerne"), result.missingIds)
    }
    ...
    fun `an episode we already hold contributes nothing`() {
        val result = computeLibraryDiff(
            ...
            ourTopLevelIds = setOf("fjollerne"),
            ourEpisodeIds = setOf("fjollerne-s11e07"),
        )
        assertTrue(result.missingIds.isEmpty())
    }
