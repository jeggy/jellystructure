...
| Android phone, Samsung Internet / Firefox / another browser without the Cast SDK | no | a **setup card**: *"Casting needs Chrome"* (Chrome is the browser Google documents; a Chromium browser that ships the SDK also works and the guide detects it rather than naming it) → one tap opens the same URL in Chrome (`intent://…#Intent;scheme=https;package=com.android.chrome;end`), then the Install step |
