...
> recently the evening of **2026-08-27 while watching Severance season 2**. A third-party technical
...
| | `Severance.S02E03…FLUX.mkv` | `Longlegs.2024…CiNEPHiLES.mkv` |
|---|---|---|
...
Severance peak video bitrate, sliding windows over the real packet stream:
...
brief ~89 Mbps spikes. Longlegs is a different animal entirely: 93 Mbps *sustained for two hours*.

**This distinction matters.** Severance is a mid-tier WEB-DL. The owner's phrase "highest quality files"
...
between them buys no headroom. Severance briefly overshoots it; **Longlegs exceeds it for its entire
...
~94 Mbps of rock-solid, jitter-free throughput. That comfortably covers Severance and everything like
it, sits *right at the wall* for a 93 Mbps Longlegs, and eliminates the entire retry/band-steering class
...
### Claim 2 — "TrueHD / DTS-HD MA demuxer overload" → **false for Severance, real elsewhere**

Severance carries **E-AC3 at 768 kbps** (§1). There is no lossless track in the file. The premise is
...
only). Longlegs is `DTS-HD MA 5.1`. So the briefing accidentally describes a genuine issue affecting
...
−59 dBm. That matches the Longlegs signature recorded 2026-08-19 (~1–4 % baseline → ~21 % during
playback).
...
The Severance file has **link count 2** — hardlinked into `/mnt/series/cross-seed-links/HD-Space/` and
...
Severance case (qBittorrent writing to `/mnt/series` while streaming from it).
