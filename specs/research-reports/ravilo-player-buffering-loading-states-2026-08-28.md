...
> Live-tested the day's Phase 177/178 delivery-aware negotiation fix against `Spindlelegs` (93 Mbps HEVC,
...
Before Phase 177, an over-decode-ceiling file (like Spindlelegs) went straight into a broken **direct
...
wall-clock time — measured live at **~20 seconds** for Spindlelegs, vs. the near-instant start of a direct
...
| 17:50:08.836 | MediaSession created — play pressed on Spindlelegs | `MediaSessionImpl` |
...
| B | **Post-handoff, pre-first-frame** — ExoPlayer has the URL and is fetching/decoding the first segment(s) of what may be a transcode still being encoded server-side | `Player.STATE_BUFFERING` before `onRenderedFirstFrame` | ❌ **Nothing.** This is the Spindlelegs gap. `PlayerScreen.kt:1254-1255` is a literal stub: `// (In a real integration the engine signals buffering; we skip this for now)`. Black screen, no spinner, no text, indefinitely. |
...
**This is binding on whatever the design produces here.** Spindlelegs is slow to start *because* it's
