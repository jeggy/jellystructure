...
**Devices:**
- **soveværelse** — Sony BRAVIA 4K VH2 (Android 12), `10.0.0.12` — *weak* panel, primary stress-test
- **stue** — Sony BRAVIA XR-65X93K (Android TV), `10.0.0.11` — *fast* panel
...
- **Pairing the soveværelse TV:** it was a fresh install on the server-address screen; pairing
  ...
  (`10.0.0.10:9505`) was entered via `adb input`, producing pairing code `X37T5G`; the
...
- **Backend:** bare `jellystructure.kexe` on the dev host (`10.0.0.10:9505`), not docker; see
