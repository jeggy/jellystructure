...
2. **The v1.13 element-size check (first `Cluster` only) flags 2 files**, both in
   *Snurre Snups Byggevenner* (Bugs Bunny Builders).
3. **A full demux pass finds far more, and it is payload damage, not layout.** On three of the five
   ...
   |---|---|---|---|
   | Byggemand Bob (1999) | 72 | **42** (all in S13–S16) | element sizes overrunning their cluster **mid-file**, invalid EBML lengths, invalid track numbers, **H.264 decode errors** (`left block unavailable`, `cabac decode … failed`) |
   | Snurre Snups Byggevenner (2022) | 93 | **20** | same set; offsets from 0x16c81 to 0x970ac47 |
   | The Cleaner (2021) | 18 | **4** (S03E02–E05) | same |
   | Taskmaster (72 files, 126 GB) · Bridgerton (32 files, 44 GB) | — | **not checked** | too much disk I/O for an evening with viewers on the box |

   Snurre Snups episodes: S00E13 S01E04 S01E06 S01E07 S01E09 S01E10 S01E13 S01E19 S01E21 S01E22 S01E26 S01E28 S01E30 S01E32 S01E33 S01E35 S01E39 S02E03 S02E33 S02E39 

   (Nine more Snurre Snups files only report `non monotonically increasing dts` on subtitle stream 5 —
...
3. **A title-scoped deep check** as a segments-lane-style job (playback-deferred per 178/213): one
   demux pass per file of one title, results into Triage. Worth building only if Taskmaster and
   Bridgerton also turn out damaged, or if the "not an ongoing writer" inference is ever contradicted.

## Addendum, same evening — the operator's own report on S01E05, checked

The household operator filed a bug report the same day on one file from this set (*Snurre Snups
Byggevenner* S01E05, `DirectPlayError` on the Jellyfin Android TV app, repaired by a stream-copy remux
...
- **The clean originals are still on disk.** The seeding copies under
  `/mnt/series/qbittorrent/downloads/` exist for Bob the Builder S13–S16 and Bugs Bunny Builders
...
- **"No quality loss" is not true of the remux.** Packet counts, clean source vs library copy:
  ...
  | Bob the Builder S13E12 source | 15 001 | | |
  | Bob the Builder S13E12 library, still corrupt | 14 859 readable (**−142**) | | |
...
- **62 of the 63** damaged Bob the Builder and Snurre Snups files (the 20 + 42 above, plus the remuxed
  S01E05) have a same-named seeding copy that demuxes with zero errors and has the same stream layout.
  The one without is Snurre Snups **S00E13**. *The Cleaner* S03E02–E05 have **no local source** and
...
- Check Taskmaster and Bridgerton at a quiet hour (≈170 GB of sequential reads).
