...
UniFi private network with static addressing (`10.10.10.x`) for their homelab. If the runner and the
...
- User's existing infra referenced in §4.2: this project's own memory
  ...
  UniFi private network, `10.10.10.x` static addressing, an always-on dev host.
