...
is 1024)` + fired the `fd_pressure` webhook during a Fumi NFO sync + Jellyfin refresh, then the process
...
- The `921` is a **60 s high-water sample** (`FdWatchdog.kt:51` `delay(60_000L)`), so it is **not** proven
  to be caused by the Fumi sync — the sync coincided with the tick. The true source accumulated over the
...
- **Latent write-path leak (fires only on write failure):** `NfoWriter.writeAtomically`
  ...
  per *failed* write. Reachable via corrupt items (Fumi S01E43, Phase 128).
