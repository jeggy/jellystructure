...
> variable-length **cold open** (Severance's title music only starts 1–5 min in, at a different
...
1. **Variable cold-open shifts the shared block out of the offset search window (the Severance case).**
   ...
   fingerprints. Severance's identical animated title sequence sits after a cold open whose length
...
must succeed for the Severance case (title sequences several minutes apart).
...
sitting behind a long cold open is still inside the analysed audio (Severance's ≤5-min cold open + a
...
## Acceptance
- On a show with a variable-length cold open (the Severance case), the improved alignment produces a
