...
> Reported as "why are all episodes 0 in Ravilo for 'La Linea'?" Investigated live against Jellyfin,
...
"La Linea" (1978, claymation shorts) has 96 files named `La Linea - 101.mkv` … `La Linea - 225.mkv`,
...
- `parseSeasonEpisodes(path)` (`Scanner.kt:68-111`) only recognizes `SxxEyy` (`SEASON_EP_RE`) or the
  ...
  run, so it falls through to `Pair(null, emptyList())` for every La Linea file.
...
- **No new filename regex.** A bare 3-digit convention like La Linea's is genuinely ambiguous on its
...
- La Linea needs a re-scan (or a `scan_files` cycle) to pick this up; already-stored `null` episode
...
2. Verified via `compileKotlinLinuxX64` and `linuxX64Test` (full suite green, incl. 4 new
   ...
   re-scan of La Linea against the live library, a scan-triggering action left for the user).
