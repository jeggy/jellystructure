...
| 64 | ✓ Done | Jellystructure startup: connection test retry — `testConnections()` runs once at shell init; if Jellyfin is momentarily unreachable the red dot stays permanently. Retry up to 3 times (5 s apart) before showing a permanent error (FR-CS1) | [phase-64](phase-64-startup-connection-retry.md) |
| 65 | ✓ Done | Activity log: oldest at bottom, auto-scroll — currently `.reversed()` puts newest at top (wrong convention); remove `.reversed()`; scroll to bottom on load; auto-scroll on new live events only when already pinned to bottom (FR-AL1) | [phase-65](phase-65-activity-log-bottom-up.md) |
| 66 | ✓ Done | Tags popup: transparent backdrop + description pre-fill + color picker CSS + library link — four bugs: `--surface` token missing; description always empty; swatch/swatches CSS only in inline `<style>` not wf.css; no "View in library" filter link (FR-TM1) | [phase-66](phase-66-tags-popup-fix.md) |
| 67 | ✓ Done | Copy command: clipboard fallback for HTTP — `navigator.clipboard.writeText()` is unavailable in non-HTTPS contexts; copy buttons silently fail. Add try/catch + `execCommand` fallback in JsInterop.kt; update permCopyBlock() (FR-CC1) | [phase-67](phase-67-copy-clipboard-fallback.md) |
| 68 | ✓ Done | Hero height minimum 30 → 40 — 4 locations: ravilo-config.html slider, RaviloConfig.kt slider + coerceIn, HomeScreen.kt coerceIn (FR-HH1) | [phase-68](phase-68-hero-height-min-40.md) |
| 69 | ✓ Done | Artwork permission banner — artwork save `Permission denied` shows only a generic "Save failed." toast; extend the proactive write-check to episode subdirectories and surface a banner with fix commands on the Artwork tab (FR-AP1) | [phase-69](phase-69-artwork-permission-banner.md) |
