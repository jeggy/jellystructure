...
| 50 | Planned | Fix Jellyfin item-refresh auth + single-item fetch — `refreshItem`'s `Authorization` header is malformed (missing `, Token=`) → 401 on every targeted refresh; `getItem` uses the unreliable non-user-scoped `/Items/{id}` endpoint and the client has no response validator, so a non-2xx `text/plain` body throws `NoTransformationFoundException` (400). Fix the header, fetch via the proven `/Items?Ids=` list shape, validate status before deserializing, and surface failures honestly (FR-JR1) | [phase-50](phase-50-jellyfin-refresh-auth-fix.md) |
