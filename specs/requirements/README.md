...
| 76 | ✓ Done | **External provider IDs (IMDb / TheTVDB) from TMDB → NFO** — Jellyfin shows the IMDb ↗ link only when the item carries an IMDb id, but Jellystructure writes only `<uniqueid type="tmdb">`. Fetch TMDB `/{movie\|tv}/{id}/external_ids` (`imdb_id`, TV `tvdb_id`) at scan + TMDB re-pull, store `imdbId`/`tvdbId` on the item, and have `NfoWriter` emit additive `<uniqueid type="imdb">` / `<uniqueid type="tvdb">` (+ legacy `<imdbid>`) — so **Save → NFO / Sync → Jellyfin** gives Jellyfin the IMDb/TheTVDB links. Reuses the existing Sonarr `getTvTvdbId`/`TmdbExternalIds` path; TMDB stays `default="true"` (FR-XI1) | [archive](archive/phase-76-external-ids-nfo.md) |
