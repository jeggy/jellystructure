...
| 53 | ◻ Planned | Scanner data-quality fixes — from a post-reset full-sync review: full-scan `year` is null everywhere (use TMDB/`ProductionYear`, not just the name); `slugify` makes empty/colliding ids that silently drop items (non-Latin titles → `""`, dup titles collide) → fall back to `jellyfinId`; widen the `SxxExx` season regex (`S2025E01` fails); surface a skipped/unmatched-items report; minor (untagged→`en` honesty, `nb`/`no`, zero-track eps) (FR-SQ1) | [phase-53](phase-53-scanner-data-quality.md) |
