...
| 17 | ◑ Reopened | Activity log backend — persistent log, filters, live runners (FR-A1). **⚠ Reopened 2026-06-25 (design-diff audit):** the "Now processing" panel is simpler than `design/app/activity.html` — missing the per-file progress bar + operation badge (e.g. "ffmpeg remux · -map 0 -c copy") and the richer per-operation status chips. *Verify the backend emits per-file progress before treating as a pure UI gap.* | [archive](archive/phase-17-activity-log-backend.md) |
| 18 | ✓ Done | Settings page cleanup — nav fix, section reorganisation (FR-C1) | [archive](archive/phase-18-settings-page-cleanup.md) |
| 19 | ◑ Reopened | Studios, Networks, Genres & Tags metadata page (FR-M1). **⚠ Reopened 2026-06-25 (design-diff audit):** diverges from `design/app/metadata.html` — the page subtitle is missing, and logo cards stack vertically (logo-on-top) instead of the design's horizontal logo-left card layout. | [archive](archive/phase-19-metadata-page.md) |
...
| 27 | ◑ Reopened | Merge Triage into Media Detail + floating Triage dock (FR-MT1). **⚠ Reopened 2026-06-25 (design-diff audit):** the Dashboard lacks the design's **"Show attention dock"** control to reopen the dock once closed (`design/app/index.html` attention-card header). | [archive](archive/phase-27-merge-triage-into-media-detail.md) |
...
| 36 | ✓ Done | Operator controls — per-library scan/push, scheduled scans, notifications (FR-OC1). Added Cron display-only field in Settings → Scanning (computed from frequency+time, readonly; backend only stores `scanIntervalHours: Int`) | [archive](archive/phase-36-operator-controls.md) |
...
| 42 | ✓ Done | Per-episode track & order editor on Series detail — same editor in a modal (FR-TO2). Season picker replaced with `.season-head` prev/next + searchable dropdown (handles 40+ seasons); "Expand all issues" button added; per-episode resolved-language badge in collapsed row; `MediaCard.watched` wired to `Tile()` across all screens | [archive](archive/phase-42-series-episode-track-editor.md) |
...
| 70 | ✓ Done | Brand mark "Quartet Play" + app logo wiring — real SVG mark on login, sidebar, topbar; connection-status row + footer auth note on login page (`Login.kt`) matching `design/app/login.html` (FR-BR1) | [archive](archive/phase-70-brand-mark-quartet-play.md) |
...
| 72 | ⌛ Planned | Canonical language-code equivalence across all comparisons — fix the false **"Default ≠ resolved"** cascade (raw `eng` track tag compared to normalised `en` resolved language → mis-fires on nearly every tagged movie). One `LanguageResolver.sameLanguage()` (B/T- and 639-1/2-agnostic) routed through every language equality check (cascade, triage mismatch, majority/mix, filters); make `normalize()` total over the picker set (+test); normalise before display; untagged tracks stay first-class. Comparison/display-only — storage/write forms unchanged (FR-LC1) | [phase-72](phase-72-canonical-language-equivalence.md) |
| 73 | ✓ Done | **Write-through metadata sync to Jellyfin** — `PATCH /api/media/{id}/metadata` and `PATCH /episodes/{ep}/metadata` saved to DB only, never writing the NFO or refreshing Jellyfin; all metadata fields (title, overview, year, tags, genres, director, studio, network) were invisible to Jellyfin after a Save. Fix: both handlers now launch `pushToJellyfin` on the app scope after `store.updateOne` (non-blocking, response returned first). NfoWriter already emits all fields correctly (FR-WM1) | [phase-73](phase-73-write-through-metadata-sync.md) |
