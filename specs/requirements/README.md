...
| 59 | ✓ Done | Restore pagebar menu/split-button CSS — the `.menu-wrap`/`.split`/`.menu`/`.menu-item`/`.mi-ic`/`.mi-sub`/`.caret`/`.split-caret` rules were removed from `app.css` in two "updated designs" commits; live app pagebar buttons are now unstyled. Add the block back to `app.css`; remove the duplicate inline copy from `media.html` (FR-MB1) | [phase-59](phase-59-restore-pagebar-menu-css.md) |
| 60 | ✓ Done | Series detail: left-rail only on overview tab — poster/identity/language card currently persists across every tab (Episodes, Artwork, NFO, History); restructure to match the movie layout (left rail inside overview panel only, other tabs full-width). Also sync `series.html` mockup + fix tab strip from `.seg`/`.active` → `.tabs2`/`.on` (FR-TD1) | [phase-60](phase-60-series-detail-tabs-layout.md) |
| 61 | ✓ Done | ⌘K command palette: media search — palette currently searches only the 9 hardcoded nav/action commands; backend `GET /api/media?search=` is fully implemented. Add async media results (≥2 chars → fetch up to 6 titles) below commands, adopt the existing unused `.cmdp-item` CSS, extend keyboard nav to cover both sections (FR-KM1) | [phase-61](phase-61-cmdpalette-media-search.md) |
| 62 | ✓ Done | Activity log: use stored timestamps — `appendLogEntry()` always calls `currentTimeString()` (browser now), discarding `entry.ts` from history load; all loaded entries show current time. Add `ts: Long?` param; format stored epoch-seconds for historical entries; keep current-time for live WS events (FR-AT1) | [phase-62](phase-62-activity-log-timestamps.md) |
| 63 | ✓ Done | Ravilo config editor: no user selected by default — editor auto-selects `users.first()` and immediately renders all sections; remove auto-select; show a "select a user" empty state until the picker is used; dropdown starts on a disabled placeholder option (FR-RC1) | [phase-63](phase-63-ravilo-config-no-default-user.md) |
| 64 | Planned | Jellystructure startup: connection test retry — `testConnections()` runs once at shell init; if Jellyfin is momentarily unreachable the red dot stays permanently. Retry up to 3 times (5 s apart) before showing a permanent error (FR-CS1) | [phase-64](phase-64-startup-connection-retry.md) |
| 65 | Planned | Activity log: oldest at bottom, auto-scroll — currently `.reversed()` puts newest at top (wrong convention); remove `.reversed()`; scroll to bottom on load; auto-scroll on new live events only when already pinned to bottom (FR-AL1) | [phase-65](phase-65-activity-log-bottom-up.md) |
| 66 | Planned | Tags popup: transparent backdrop + description pre-fill + color picker CSS + library link — four bugs: `--surface` token missing; description always empty; swatch/swatches CSS only in inline `<style>` not wf.css; no "View in library" filter link (FR-TM1) | [phase-66](phase-66-tags-popup-fix.md) |
| 67 | Planned | Copy command: clipboard fallback for HTTP — `navigator.clipboard.writeText()` is unavailable in non-HTTPS contexts; copy buttons silently fail. Add try/catch + `execCommand` fallback in JsInterop.kt; update permCopyBlock() (FR-CC1) | [phase-67](phase-67-copy-clipboard-fallback.md) |
| 68 | Planned | Hero height minimum 30 → 40 — 4 locations: ravilo-config.html slider, RaviloConfig.kt slider + coerceIn, HomeScreen.kt coerceIn (FR-HH1) | [phase-68](phase-68-hero-height-min-40.md) |
| 69 | Planned | Artwork permission banner — artwork save `Permission denied` shows only a generic "Save failed." toast; extend the proactive write-check to episode subdirectories and surface a banner with fix commands on the Artwork tab (FR-AP1) | [phase-69](phase-69-artwork-permission-banner.md) |
