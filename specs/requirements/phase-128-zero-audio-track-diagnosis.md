...
Reported case: **Mumi S01E43** shows English audio and can't be changed to Faroese.

- The stored episode (`config/jellystructure.db`, `mumi-1990`) has **0 tracks total**, `resolvedLanguage
  = en`, `issueCount = 0`. Series is otherwise `fo`.
- Running ffprobe on the real file
  `/mnt/series/jellyfin/Mumi/Season 01/Mumi - S01E43 - Sirkusprinsessan.mp4` (638 MB, exists, readable,
...
Library-wide scope confirmed rare: **2 items** have zero audio tracks (Mumi S01E43; one unmatched
...
- **Not every file is \*arr-managed** — Mumi S01E43 was a manual download. `Re-probe file` is the
...
## Acceptance
- Open Mumi → S01E43 → "Tracks & order": empty-state card with the corrupt-file reason, the **copyable
  file path**, and a `Re-probe` button — and because Mumi is **unmanaged**, manual-repair guidance (no
...
- No regression on the 46 healthy Mumi episodes (`primaryAudioLanguage` still returns the primary
