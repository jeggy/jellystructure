...
`✓ Built` 2026-09-17 — **server:** `RowSort` + `RowConfig.sort/pinned/limit`, `Row.sort_by/sort_descending`, the shared `RowOrder` resolver (`shared/…/RowOrder.kt`) replacing the seven hard-coded sorts (the hero's stays), `SortName` fetched in all six scanner `Fields=` lists and stored on `MediaItem` (no `.sqm`), validation in `RaviloConfigService.validate()`, pins deduplicated in `normalize()` with an absent sort left absent. `RowOrderTest` (11) incl. *an absent sort is today's order exactly*; backend 365/0, every existing feed test unchanged. **Editor:** the Order section as `WorkbenchOrder.kt` — four-way seg + direction in words, *Show N titles* stepper that will not step below the pin count, the strip with the dashed seam (click / drag across to pin or release, drag within to reorder, ✕, *Pin a title…*), *Then the rest by*, the stale-pin note naming the collection, the ceiling line and toasts, the Matches panel numbered and faded past the limit — previewed with the **same** `RowOrder.resolve` the server serves with; all four row editors wired; the row list says the order in words. Admin wasm compiles. **Verified 2026-09-17 (evening) on a throwaway local v1.22 instance over a copy of the production config + DB**, server by API and editor in headless Chromium: the four bad writes answer 400 with their sentence (bad key, limit 2, pins > limit, an order on *Continue Watching*); a *title A → Z · 2 pins · show 8* row arrives in `/api/tv/home` with the two pins first, `sort_by = title`, and **no pin list on the wire**; a stale pin stays in the saved config; restoring the config returns the identical 30-tile row (acceptance 1). In the editor: the four-way control, direction in words, the strip with its seam, numbered picks, the limit stepper, *Then the rest by*, *Pin a title…*, the row-list summary (*2 hand-picked, then newest release first · shows 15*) and reopen-restores-state all work. **Three defects found by that pass and fixed:** the preview showed only 30 of 89 matches (the shared resolver clamped the editor's uncapped preview — now `RowOrder.resolveAll`; and the list endpoint pages at 100 — now paged), *titles* wrapped onto its own line, and toasts were wiped by the re-render that follows them. Live on production as v1.22 (server half); the three editor fixes ship with the next release. Was `Planned` — written 2026-09-17, **dev-reviewed 2026-09-17 against `main` `8873cea7`** (see §Dev review at the
