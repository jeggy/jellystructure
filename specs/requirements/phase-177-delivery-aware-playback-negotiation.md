...
> Vision and plain-HEVC paths — yet `Longlegs` (93 Mbps average, DTS-HD MA) direct-plays into them
...
- **A safety margin applies.** The declared ceiling is the device's reported value scaled by a constant
  ...
  does not account for, and for the short peaks a rolling average hides — Severance averages 24 Mbps but
...
1. **RESOLVED (2026-08-28, live).** Posted `PlaybackInfo` directly at the real Jellyfin 10.11.11 instance
   for the library's own Longlegs (HEVC 93.3 Mbps HDR10 + DTS-HD MA 6ch) with a `DeviceProfile` matching
...
2. **RESOLVED (2026-08-28, live, same test run).** Re-ran with the `hevc` `VideoBitrate` condition set to
   60 Mbps (below Longlegs' own probed `BitRate` of 93,375,771) and got a REAL video re-encode this time
