...
Klovn's row, production, 2026-09-06 08:40 local:
...
`scan_files` is configured `refresh_this_year = "daily"`, and Klovn correctly resolves to that tier —
...
`now - last_checked` is never ≥ 24 h, so Klovn is **never due**:
...
Klovn is one of the 504. A new episode had been on disk for two hours.
...
(`Wrote NFO: /mnt/series/jellyfin/Klovn/tvshow.nfo`, `2026-09-06T00:10:14Z`). Not one of those 310
...
The sweep detected Klovn's missing episode on the first cycle after it landed. The ingest path it feeds
...
- `specs/requirements/phase-181-converge-on-jellyfin.md` — FR-181-2 and the original Klovn incident
  this phase's finding re-opens from the other end.
- Production evidence 2026-09-06: `klovn-2005` (`scanned_at` 2026-09-05 01:49:11 vs `last_checked`
