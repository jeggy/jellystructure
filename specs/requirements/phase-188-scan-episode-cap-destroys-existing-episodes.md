...
> Live bug report: Klovn (11 seasons, ~99 episodes) showed exactly one episode per season in the admin
> and in Ravilo. Reported as "seems like a general bug, not Klovn-specific" — confirmed against the
...
still carrying in-window timestamps are all Breaking Bad, and every one matches its real file mtime or
...
- **Root cause confirmed directly against the prod DB**, not guessed: `klovn-2005`'s stored `episodes`
  ...
  runs (verified by title — e.g. `X Factor` 2 seasons, `#CyberSleuths` 3-part miniseries); only 3 exceed
