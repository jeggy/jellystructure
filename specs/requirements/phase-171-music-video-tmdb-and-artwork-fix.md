...
> works for musicvideos? Example `a73985678a02866af471dc597ca42eaa` is Vesper Orbit, but this was a live
...
the real library**, an artist's folder holds multiple music videos side by side (e.g. `Ranvá/` has 4).
Real, pre-existing poster files were already sitting on disk under the per-basename convention this
house's library already used — `<video-basename>-poster.jpg` (e.g. `Ranvá - Live in Bergen (2026-08-04)-poster.jpg`) — the same shape `episodeStillPath`/`NfoWriter.nfoPath` already use
...
movie entry — confirmed live: "Vesper Orbit Tour" (filename) has no TMDB match by that title, but the
same file genuinely is TMDB movie **25352**, "Vesper: ORBIT - Live from Parken Stadium" (2008). The
...
- `scanMusicVideo` (`Scanner.kt`): mirrors `scanMovie`'s TMDB half — `jItem.providerIds?.tmdb ?:
  ...
  all for the bare filename-derived "Vesper Orbit Tour", but the artist-qualified real title matches).
...
- Live-verified against the real library: confirmed the exact reported item
  (`jellyfinId=a73985678a02866af471dc597ca42eaa`, "Vesper Orbit Tour") is TMDB movie 25352 via a direct
