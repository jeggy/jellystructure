...
| fingerprint | fingerprint | duration not stored | — | 172 | same two shapes (La Linea, 3-minute files) |
...
the other as `credits`, at the same timestamp (Bananer i pyjamas S01E02: intro 333 713 ms, credits
...
Evidence rows on production: **68 accepted** `Opening Credits` chapters between 61 s and 435 s (It's
Always Sunny in Philadelphia: 104 s). Sixteen more were saved only by the 60 s guard.
...
3. Bananer i pyjamas S01E02 ends with a credits marker near 333 s and either a first-half intro or none.

## Open questions

1. **Seven item ids in `media_segment` no longer exist in `media`** (`bob-the-builder`, `babblarna`,
...
2. Two-cartoon files (New Looney Tunes: a title card at 638 s of 1 320 s) have a mid-file "intro"
