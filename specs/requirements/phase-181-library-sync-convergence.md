...
### 2.5 — confirmed live again 2026-08-31 (Beacon Watch S01E03): the series-level skip blocks *local file
discovery*, not just the expensive steps

Reported as *"i cant see beacon-watch in my newly added section"*. Investigation found the show itself was a
red herring — Beacon Watch S01E01/E02 were already scanned and the series correctly ranked #15/30 in the live
...
itself had scanned and identified the file — while jellystructure's `media` row for Beacon Watch still held
only 2 episodes.

This is not a Jellyfin-lag case like Fjollerne (§2.1) was — Jellyfin was not the bottleneck here.
FR-181-2 (✅ built) worked exactly as designed: `sonarrNextAiringDate = 2026-09-07` correctly bucketed
Beacon Watch into `refresh_this_year` (daily), not the old monthly archive tier. **And it still didn't help**,
...
Beacon Watch' series row had `last_checked` ≈20 minutes before S01E03 finished downloading, so the very next
...
series dirty (FR-181-5) and forces a rescan irrespective of cadence. Fjollerne was `100` vs Jellyfin's `101`
