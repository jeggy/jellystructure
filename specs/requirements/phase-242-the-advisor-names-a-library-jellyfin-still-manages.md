...
3. Does a music library need different treatment? `Musik` holds music videos per phase 168, and
   `MediaKind.MUSIC_VIDEO` never uses TMDB. Whether its metadata story is the same one is worth
   settling before writing its findings.
