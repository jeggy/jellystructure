...
`c045ed9c87a18fbb1580cfed748acb86` is 열배's Jellyfin id; the item is `tenfold-2003`. Read from
...
titlesByLang:          { "en": "Tenfold", … }   ← the English title is right there, in the same row
```
...
1. Set 열배's metadata language to English; confirm `title = "Tenfold"`.
2. Run a full library scan (or `POST /api/media/tenfold-2003/sync`). The title is **still** `Tenfold`,
...
4. `POST /api/media/tenfold-2003/repull-jellyfin`: same result.
