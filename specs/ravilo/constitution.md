...
- **Android TV target:** native Android app, `leanback` launcher category, TV banner, D-pad primary
  input, **Media3 (ExoPlayer)** for playback.
...
- **Player:** a shared `expect`/`actual` **`RaviloPlayer`** interface — `actual` = Media3 on Android,
  `actual` = browser video on Web. The only sanctioned engines for each platform.
