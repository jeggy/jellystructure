...
Verifying R235 on `Kulsort.S07E03` (the exact file from that phase's live report), the picker's
**level 1** row read:
...
1. On `Kulsort.S07E03`, level 1's Danish row shows `Sound described` — not `Default` / `Signs only` —
