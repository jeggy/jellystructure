...
| R44 | Planned | **Media transport keys** — the remote's physical Play/Pause/Stop/FF/Rew/Next/Prev buttons are inert (only on-screen transport works). Route `Key.Media*` to the existing player actions (`togglePlay`/`skip`/`advanceNext`), attach a Media3 `MediaSession` to the Android ExoPlayer so the OS delivers hardware transport keys, and wire `navigator.mediaSession` action handlers on web | [R44](phase-R44-media-transport-keys.md) |
