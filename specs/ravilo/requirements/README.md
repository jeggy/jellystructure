...
| R46 | Planned | **Human-readable track labels** — the Audio/Subtitles picker shows bare codes ("DAN") instead of Jellyfin's rich `DisplayTitle` (e.g. "Synstolkning") because the `StreamTicket` carries no audio metadata and the player labels from container `Format.label`/language only. Add an `AudioTrack` list to the ticket sourced from Jellyfin `MediaStreams`, prefer the backend label in the picker (mapped by track order), and humanize the language fallback. Follow-up to R41 | [R46](phase-R46-track-label-metadata.md) |
