...
| R45 | Planned | **Entry scroll + focus restore** — screens open scrolled to a mid-page button and never re-frame the hero (Home hero, Movie Play, Series Resume all auto-scroll via `playFR.requestFocus()` bring-into-view), and content rows lose their `trackPadH` left inset after scrolling in and back. Make "top action focused ⇒ scrolled to top / hero framed", and restore a row's leading inset when its first item is focused — without regressing R42/R43 smoothness | [R45](phase-R45-entry-scroll-focus-restore.md) |
