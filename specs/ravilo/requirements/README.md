...
| R38 | Planned | **Launcher-icon optical centering** — the jellyfish reads low in the TV Apps overview (top-heavy dome + thin tentacles → geometric centre below optical centre); nudge the mark up a few % when compositing and regenerate the icon pack. Master SVG + inline marks unchanged. Follow-up to R37 | [R38](phase-R38-launcher-icon-optical-centering.md) |
| R39 | Planned | **Channel logo fills the button** — an uploaded channel logo covers the whole button (cover / no padding), clipped by the rounded corners, consistent on TV + admin preview/row + mockup. Follow-up to R36 | [R39](phase-R39-channel-logo-fill.md) |
| R40 | Planned | **Instant back navigation** — Back re-shows a loading skeleton because `AnimatedContent` disposes the screen + its store; retain store state/cache so re-entry is instant with silent background refresh (Home/Channel/Browse/Detail/Search) | [R40](phase-R40-instant-back-navigation.md) |
| R41 | Planned | **TV subtitle tracks (always-zero bug)** — source subtitles from Jellyfin `PlaybackInfo`, fix the malformed (duplicated-id) subtitle URL, and make subtitle selection actually work (`TrackSelectionOverride` / external config); picker lists embedded + external | [R41](phase-R41-tv-subtitle-tracks.md) |
| R42 | Planned | **Fix focus-animation viewport jump** — stop bring-into-view re-centring already-visible tiles, reserve scale-overflow space (`trackPadV`), keep the scale draw-only; smooth focus with no screen jump | [R42](phase-R42-focus-animation-viewport-jump.md) |
