...
> A viewer reported "Contact Week" (2160p DV profile 8 / HDR10+ hybrid) refusing to play in Ravilo
...
## Bug report
2026-07-25 — playing "Contact Week" on Ravilo (Android) never produced a frame; Jellyfin logged
...
- The file: `Contact Week (2026) (2160p iT WEB-DL Hybrid H265 DV HDR10+ DDP Atmos 5.1 …).mkv`, 26.6 GB.
...
- Live negotiation matrix (each row re-run for this spec):
  ...
  |---|---|---|---|
  | Contact Week | `DOVIWithHDR10Plus` (p8) | transcode → `avc1.424029` @3840x1606 (undecodable) | **direct play** |
  | The Rascals 2 | `DOVIWithHDR10` (p8) | transcode | **direct play** |
  | In Your Sleep | `DOVIWithHDR10` (p8, **AV1**) | transcode | **direct play** |
  | Mesmeric / The Governess | `DOVIWithEL` (p7, dual-layer) | transcode, undecodable | transcode → `avc1.640033` (decodable) |
  | undertone | `HDR10Plus` | direct play (unchanged) | direct play |
  | 40 Days Gone | `SDR` h264 | direct play (unchanged) | direct play |
  | Contact Week, SDR-only client (ravilo-web) | `DOVIWithHDR10Plus` | transcode, undecodable | transcode → `avc1.640033` @1920x803, tone-mapped |
