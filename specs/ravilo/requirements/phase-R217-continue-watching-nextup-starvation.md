...
> User report, live, 2026-08-28: watched Severance S02E05 to completion the evening before, but the
...
2. Fetch every next-up item — a series whose most recent episode was *finished*, now waiting on the next
   one (Severance's exact situation: finishing S02E05 drops it off "resume" and moves it to "next-up" for
   S02E06 "Attila").
...
- Live count (jogvan's profile, 2026-08-28, via the exact request `buildContinueRow`/`getResumeItems`
  ...
  bursts (Bluey, Smábørn, Teletubbies, Peter Pedal…) sitting alongside adult in-progress titles.
- `getResumeItems`/`getNextUp` themselves are healthy and NOT the bug — both already fetch a 200-item
  candidate pool (R186) and Jellyfin returns Severance correctly (next-up position 4, `SeriesId` matches
...
NextUp` ordering — empirically confirmed recency-biased in this same live test: Severance's neighbours in
...
- With today's numbers this alone is enough: Severance sits at next-up position 4, so a strict 1:1
