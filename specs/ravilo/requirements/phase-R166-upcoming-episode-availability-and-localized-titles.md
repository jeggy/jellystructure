...
series use the majority of episodes' resolved languages), so a Danish-audio "Taskmaster (DK)" is stored
as `MediaItem.title = "Stormester"`. The Upcoming feed throws that away and shows the *arr's English
...
5. When a feed item matches a catalogue `MediaItem` (`matched != null`), the `UpcomingItem.title` must be
   the library's **localized** `matched.title` (e.g. "Stormester"), not the *arr English string. Falls
