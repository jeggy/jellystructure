...
1. **The HTTP layer.** `respondCachedBytes()` sends `Cache-Control: max-age=86400` (24h) with
   ...
   `/api/tv/image/hus-2026/poster` will not even issue a conditional request for up to 24h — it has no
...
- `compileKotlinLinuxX64`, `:ravilo-ui:compileKotlinWasmJs`, `:ravilo-ui:compileDebugKotlinAndroid` all
  clean.
- Confirmed against the live `hus-2026` item pre-fix: backend-side artwork and its resize cache were
  already correct (`poster.jpg.src` matches `item.posterPath`; `hus-2026-poster.ct` recorded the current
