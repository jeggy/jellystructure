...
> A viewer reported "Undertone" (HEVC 10-bit HDR10+, confirmed via `ffprobe`) played very dark on Ravilo
...
## Bug report
2026-07-05 — streaming "Undertone" in Ravilo looked very dark; the same title played perfectly through
Jellyfin's own client.

## Investigation
- Confirmed the file is genuine HDR: `ffprobe` on the stored path
  (`/mnt/media/jellyfin/movies/undertone (2026)/undertone.2025.2160p.iT.WEB-DL.DDP5.1.Atmos.HDR.H.265-SCOPE.mkv`)
...
## Acceptance (verified)
- `ffprobe` confirms "Undertone" is genuine HDR10+ (PQ/BT.2020/10-bit).
